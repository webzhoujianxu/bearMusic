        ...
        <div class="hotMusicList">
            <div class="hotMusicList-item" v-for="(item,index) in MusicList" v-bind:key="item.value">
                <div class="hotMuisc-index" v-if="MenuType==1&&showIndex">
                      <span v-html="index<9?'0'+(index+1):index+1" class="Nums" v-bind:class="index<3?'hotIndex':'normalIndex'"></span>
                </div>
                <div class="hotMusicList-left">
                    ...
                    <p class="hotMusicList-left-tag" v-if="MenuType==1">
                        <img src="../../BaseMenu/index_ic.png" class="tag" v-if="item.cd!=1"> <span v-for="itemd in item.ar" style="margin-right: 5px">{{itemd.name}}</span> - {{item.name}}</p>
                </div>
